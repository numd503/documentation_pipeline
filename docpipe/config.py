...
def resolve_input(value: str | Path, config: Path | None) -> Path:
    """Путь к файлу или каталогу, названному ключом `docpipe.yaml`.

    Сначала от текущего каталога, потом от каталога самого `docpipe.yaml`.
    Порядок именно такой, а не обратный: конфигурации, лежащие в корне
    репозитория, писались с путями от текущего каталога, и менять под ними
    смысл значения нельзя — путь, который вчера указывал в существующий
    каталог, завтра указывал бы в другой, и прогон бы не упал, а взял не тот
    набор шаблонов.
    ...
    Не найдено нигде — возвращается первый кандидат: сообщение об ошибке должно
    называть то, что человек написал в конфигурации, а не последнее из того,
    что перебрал инструмент.
    """
    candidates = list(candidate_inputs(value, config))
    return next((path for path in candidates if path.exists()), candidates[0])


def candidate_inputs(value: str | Path, config: Path | None) -> list[Path]:
    """Кандидаты для `resolve_input` по порядку. Нужны отдельно, чтобы
    сообщение об ошибке могло перечислить всё, что пробовали: «не найдено»
    с одним путём заставляет гадать, искался ли второй."""
    here = Path(value)
