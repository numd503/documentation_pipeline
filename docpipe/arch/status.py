...
**Всё, что лежит в файле, — снимок.** Даже запись с провенансом `adapter`:
провенанс говорит, кто её добыл, а не читается ли она заново. Живые записи
в файле не лежат вовсе — их производит адаптер при каждой сборке, и стареть
им нечем. Первая редакция считала `provenance: adapter` признаком «живой»
и не проверяла такие записи на устаревание — то есть молча не проверяла
ровно те, что легче всего снять снимком.
...
STATES: Final[tuple[str, ...]] = ("stale", "source_missing", "no_hash", "current")
