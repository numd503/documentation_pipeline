...
def _extract_xml(elem: ET.Element, expr: str) -> str | None:
    """Достать значение по выражению: `#tag`, `@attr`, `путь/@attr` или `путь` (текст).
    ...
    `#tag` — имя самого элемента. Нужно там, где вид записи закодирован тегом,
    а не атрибутом: у полей списка структуры это `FieldText`, `FieldLookup`,
