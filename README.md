...
Готово 8 задач из 23 (см. [журнал реализации](docs/implementation-log.md)):

| | |
|---|---|
| ✅ T00–T03 | каркас, тестовые решения, примитивы детерминизма, модели данных |
| ✅ T04–T05 | обход ФС, разбор `.csproj` / `.sln` |
| ✅ T06 | разбор объявлений типов через tree-sitter |
| ⬜ T07–T13 | члены типов, кэш, резолв FQN, маршруты, DI |
| ⬜ T14–T21 | классификация, сборка дерева, детерминизм, скоуп-режим, CLI |

134 теста, 1000 строк кода.
...
Парсинг выполняется грамматикой `tree-sitter-c-sharp` вместо Roslyn. На реальном
репозитории (Microsoft eShopOnWeb, 244 файла) — **0 ошибок разбора, 2539 файлов/с**.
Roslyn потребовал бы .NET SDK, `restore` и сборку, то есть минуты вместо секунд.
...
└── dotnet/
    ...
    ├── parser.py         C# -> FileParseResult
    └── queries/*.scm     запросы tree-sitter
...
| [`docs/findings-eshoponweb.md`](docs/findings-eshoponweb.md) | отчёт о прогоне на реальном репозитории |
