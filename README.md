...
| `--stats` | счётчики и подсказки по правилам; **ничего не пишет** |
...
### Шаг 0. Прогнать как есть и посмотреть на цифры

```bash
uv run docpipe scan --root . --out artifacts/doc-tree.json
cat artifacts/doc-tree.run.json
```

Три числа в `stats` задают всю дальнейшую работу:

```json
"stats": { "symbols": 8966, "classified": 828, "excluded": 996, "unclassified": 7142 }
```

- **`excluded`** — отсеяно секцией `exclude`. Если сюда попало слишком много, правила
  отсева слишком жадные.
- **`unclassified`** — не подошло ни под одно правило. Это и есть главный сигнал:
  пока число большое, вы не описали словарь проекта.
- **`classified`** — то, что станет документами.

### Шаг 1. Посмотреть, чего не хватает

Счётчик говорит «5013», но не говорит, **какие** правила писать. Для этого есть `--stats`:

```bash
uv run docpipe scan --root . --stats
```

Команда ничего не пишет — только печатает таблицу и пять срезов по неклассифицированным
типам: модули, окончания имён, базовые типы с учётом наследования, атрибуты, namespace.
На ABP это выглядит так:
...
Секция `exclude` в `rules/dotnet.yaml` работает **до** правил.

**Главный рычаг здесь — `require_public`.** Замер, какая причина отсева срабатывает чаще:
...
- **`unclassified` перестал падать** при добавлении правил — словарь описан;
