...
кластера) и `templates/` (скелеты документов шага 2). Сам инструмент — уровнем
выше и правке не подлежит: его обновляет `install.sh`. Ваши правки в трёх
названных местах он сохраняет, новая версия приходит рядом как `*.new`.
...
docpipe docs owners  $OUT --root . --config $BUNDLE/docpipe.yaml --lint
```
...
Без `ownership.yaml` у всех документов `team: null`: работать это не мешает,
но `--team` ничего не отбирает, а `docs owners --lint` покажет весь манифест
ничьим. Файл заводится отдельно, путь к нему — закомментированный ключ
`ownership` в `docpipe.yaml`.
