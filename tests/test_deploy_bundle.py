...
def test_bundle_config_names_the_templates_directory() -> None:
    ...
    Вторая ступень поиска — каталог самого `docpipe.yaml` — сделала бы рабочим
    и короткое значение. Проверка остаётся: явный путь читается одинаково из
    любого каталога, и опираться на ступень там, где можно не опираться, незачем.
    """
