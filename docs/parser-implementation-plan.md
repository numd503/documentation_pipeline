...
> Секция `exclude` переделана в **T23**: у каждого решения появились `id`, `reason`
> и условие из тех же десяти предикатов. Краткая форма из четырёх полей, приведённая
> ниже, продолжает работать — но новые наборы пишутся по T23.
...
> Формат отчёта из этой задачи заменён в **T23**. Всё остальное — `--dry-run`,
> `validate`, четыре инварианта манифеста — в силе.
...
> **Переделано в T23.** Счётчик `unclassified` переименован в `undecided`, `excluded` —
> в `not_documented`, а отчёт перестроен вокруг вопроса «принято ли решение». Формат
> вывода, описанный ниже в этой задаче, действовал до T23 — читать его как историю,
> а реализовывать по T23. Причина смены — в самой T23.
...
`cashflow-docspipe/{docpipe.yaml,rules.yaml,README.md}`), `tests/test_deploy_bundle.py`
...
3. **Сборка самого пакета — необязательный шаг.** Если зеркало не отдаёт
   ...
   требование к документированному вызову: звать `python -m docpipe`
   с `PYTHONPATH` на каталог поставки, а не консольную команду `docpipe`, —
   тогда оба режима работают одинаково.
4. **Запуск не ходит в сеть** (`uv run --no-sync`). Иначе каждый запуск сверялся бы
   с индексом и падал бы там, где индекс недоступен, — на команде, которая
   с зависимостями ничего не делает.

> **Изменено в T25.** Пунктами 3 и 4 сначала распоряжался скрипт-обёртка
> `cashflow-docspipe/run.sh`. Скрипт удалён: он дублировал CLI своим набором
> режимов, и требования переехали в инструкцию, где они видны как переменные
> окружения и флаги. Проверяются они теперь по тексту инструкции — см. T25.
...
`registry = "https://pypi.org/simple"` и вместо копирования пересобирает — но
только если изменился состав зависимостей (сравнением `pyproject.toml`). Иначе
обновление одного лишь кода требовало бы связи с зеркалом на ровном месте.

**Зеркало может отдавать не всё.** Тогда набор подбирается руками внутри контура,
а результат — `pyproject.toml`, `uv.lock`, `uv.toml` — возвращается в `deploy/`
клона, и дальше установка идёт без подбора (`deploy/OFFLINE.md`). Отсюда два
требования:

- установщик берёт `deploy/uv.toml`, если он есть, — тогда `--index` больше
  не нужен ни на одной машине периметра;
- сверка манифестов в тесте сравнивает **имена** зависимостей, а не ограничения
  версий: на закрытом контуре ограничения расходятся законно, подстраиваясь под
  то, что есть в зеркале. Забытая при добавлении зависимость при этом ловится
  по-прежнему.

Пути в документации и скриптах — через `$WORK` (`/home/work/<логин>`), переменную
целевой системы. Захардкоженный `/home/work/$USER` неверен там, где логин
и имя каталога различаются.
...
- документированный вызов из установленного каталога строит манифест, `validate` его
...
- `--no-install-project` даёт рабочую установку из 15 пакетов **без** консольной
  команды `docpipe`, и вызов через `python -m docpipe` в ней работает так же;
...
**Цель:** отчёт `--stats` отвечает на вопрос «про что решение ещё не принято», а не
«чего не хватает правилам». Настройка набора становится задачей с признаком завершения.

**Изменить:** `docpipe/classify.py`, `docpipe/stats.py`, `docpipe/cli.py`,
`docpipe/emit.py`, `rules/dotnet.yaml`, `tests/test_classify.py`, `tests/test_stats.py`

### Зачем

До этой задачи отчёт не мог сказать «готово», и не по недосмотру, а механически.

1. **Решение «не документируем» было анонимным.** Счётчик `excluded 996` не говорил,
   по какому решению и зачем. Разбивка по причинам, которая есть в README
   («на ABP `name_regex` — 508 из 996»), добыта ручными замерами: инструмент её
   не показывал.
2. **Большинство решений было негде записать.** Секция `exclude` умела ровно четыре
   условия: `path_glob`, `name_regex`, `type_kind_deny`, `require_public`. Решение
   «типы с атрибутом `[GenerateCode]` не документируем» выразить было нечем, и на АС CF
   эти **1478 типов** оставались в `unclassified` навсегда — сколько бы раз человек
   их ни просмотрел глазами.

Отсюда следовало главное: `unclassified` смешивал «ещё не смотрели» с «посмотрели
и решили, что не надо», потому что второе физически некуда было положить. Число
оставалось большим всегда, поэтому по нему нельзя было ни увидеть завершение
настройки, ни заметить появление нового типа в репозитории.

### Модель: три исхода, и все три — решения

| Исход | Где записано | Счётчик |
|---|---|---|
| документируем | правила в `rules.yaml` | `<kind>…` |
| не документируем | `exclude` в `rules.yaml` | `not_documented` |
| **решение не принято** | нигде | `undecided` |

Плюс два состояния, которые решением человека не являются и потому в отчёте стоят
отдельно: `not_enrolled` (решено уровнем выше, в `docpipe.yaml`) и `interface_covered`
(решил инструмент).

**Гранулярность решения — группа, а не символ.** Перечислять 4100 FQN нельзя: список
сломается на первом переименовании и создаст работу там, где смысл не менялся. Довод
тот же, что у якорей бизнес-слоя и у `business_hash`. Решение = предикат + причина,
и таких на репозитории выходит 10–20, а не 4100.

### Секция `exclude`: список правил

Форма та же, что у правил классификации, и движок тот же (`ruleset.py`), поэтому все
десять предикатов доступны сразу и без нового кода:

```yaml
exclude:
  require_public: true          # остаётся полем, см. ниже
  rules:
    - id: generated.schemas
      reason: "Сгенерировано по XSD сервисных контрактов"
      priority: 100
      when:
        any:
...
`reason` обязателен: без него «не документируем 4820» снова становится безымянным
числом, то есть ровно тем, от чего задача уходит.

**`require_public` остаётся полем, а не правилом.** Причина техническая и содержательная
сразу. Отрицания в движке нет намеренно, и «не public» через его предикаты не
выражается; а по смыслу это не решение о группе типов, а глобальный переключатель
«документируем ли мы внутреннюю кухню». В отчёте он участвует как псевдоправило
`exclude.require_public` с приоритетом ниже всех остальных.

**`unless` — единственная форма отрицания, и она привязана к правилу.** Правило
отсева совпадает, когда `when` истинно и `unless` ложно. Нужно это там, где
в каталоге лежит один вид документируемых типов и много вспомогательных,
а описать вспомогательные положительно не выходит: «исключаем каталог, кроме
наследников `GridService`».

> **Ловушка, на которой спотыкаются при настройке: `exclude` — стадия ДО правил,
> а не участник соревнования с ними.** `classify` сначала спрашивает
> `is_excluded` и при истине выходит, не дойдя до `rules`. Поэтому приоритет
> правила классификации исключающее правило не отменяет **никогда**, сколько
> его ни поднимай: приоритеты двух стадий не сравниваются. Настройщик,
> исключивший каталог целиком и поднявший `priority` у нужного правила, получает
> пустую документацию и не понимает, почему. Отсюда же второе, менее очевидное:
> **чтобы тип не документировался, исключать его не обязательно** — узел
> создаётся только при совпадении с правилом из `rules`. `exclude` нужен, чтобы
> отличить `not_documented` от `undecided`, а не чтобы «выключить» тип.

Общего `not` внутри дерева условий по-прежнему нет, и это не изменилось: он
превращает набор в систему уравнений, которую нельзя прочитать построчно.
`unless` привязан к одному правилу и описывает вырез из **его** группы — форма
читается сверху вниз. Пустой `unless` отвергается при загрузке: это недописанное
правило, а не «исключений нет», и молча оно вело бы себя как обычный отсев.
Причина у группы остаётся одна, поэтому отчёт `--stats` от выреза не дробится.

**Краткая форма из четырёх полей продолжает работать.** Она разворачивается в правила
с зарезервированными идентификаторами `exclude.<поле>` и общими причинами; формы
складываются, поэтому `rules:` можно дописать рядом с полями, ничего в них не меняя.
Это не любезность, а требование: копия набора уже лежит на боевом репозитории АС CF,
и правка не должна требовать одновременной миграции.
...
Решения по 8966 символам:
  ...
  не документируем            4820  9 решений
  вне области                 2141  enrolled в docpipe.yaml
  интерфейс с реализацией       12  решил инструмент
  ------------------------------------
  РЕШЕНИЕ НЕ ПРИНЯТО          1119  <- это и есть работа
...
Решение не принято — за что зацепиться (топ):
  <пять срезов из T20 без изменений>
```

Что здесь существенно:

- **значима последняя строка блока решений.** Всё выше неё решено и перечитывать это
  не нужно. При нуле строка остаётся на месте с подписью «решены все символы»:
  исчезнув, она заставила бы выяснять, всё решено или отчёт сломался;
- **нулевые категории не показываются** — категория без символов уводит взгляд
  от той, в которой они есть;
- **таблица видов не повторяет служебные категории** и не печатает `total`: сумма видов
  меньше числа символов, и итог в этой таблице читался бы как ошибка. По манифесту
  (`docpipe stats`) итог печатается — там он равен сумме;
- **срезы остались ровно теми же**, но переехали внутрь нерешённого: они инструмент
  принятия следующего решения, а не отчёт о состоянии.

### `--fail-on-undecided`

Флаг у `scan`: код 1, если `undecided > 0`. Ради него всё и переделано. Пока «не решено»
было счётчиком «неклассифицировано», он был большим всегда, и новый тип в репозитории
в нём не выделялся; доведённое до нуля «не решено» превращает такой тип в упавшую сборку.

### Ловушки

**Атрибуция при нескольких совпавших решениях — по `priority`, затем по `id`**
(`pick_winner`), и никогда по порядку в файле. Иначе перестановка двух правил в YAML
меняла бы цифры в отчёте, а они идут в журнал и в CI. Правила краткой формы получают
отрицательные приоритеты, чтобы решение, написанное человеком, забирало атрибуцию
себе: его причина информативнее общей.

**Правило-заглушка обнуляет нерешённое, ничего не решив.** `when: {}` отвергается при
загрузке само (условие обязано быть словарём с одним ключом), но широкое условие вроде
`path_glob: ["**"]` с причиной «разберусь потом» синтаксически законно и запретить его
нельзя — законные широкие решения существуют. Видно это только в таблице причин:
по одному решению с неправдоподобно большим счётчиком. Поэтому таблица и сортируется
по убыванию счётчика.

**Префикс `exclude.` зарезервирован** за развёрнутой краткой формой. Без запрета
пользователь может назвать своё правило `exclude.name_regex`, и в таблице причин
окажутся две строки с одним id.

**Опечатка в имени поля секции `exclude` обязана быть ошибкой загрузки.** `name_regexp`
вместо `name_regex` иначе означает молча не работающий отсев — то же самое, что опечатка
в имени предиката, и лечится так же.

**Критерии приёмки**
- манифест по фикстуре отличается от эталонного **ровно одной строкой** —
  `ruleset_version`. Это и есть доказательство, что перевод эталонного набора в новую
  форму ничего не изменил по существу;
- состав отсева на `SampleSolution` и `WildSolution` совпадает у краткой формы
  и у формы с причинами (тест `test_reference_set_migration_changed_nothing`);
- `--stats` на фикстуре печатает `Решения по 10 символам`, строку `РЕШЕНИЕ НЕ ПРИНЯТО 1`
  и таблицу причин с решением `data.contracts`;
- `--fail-on-undecided` на фикстуре → код 1 и сообщение «Решение не принято
  по 1 символу»; на наборе, где решено всё, → код 0;
- сумма всех счётчиков по-прежнему равна `total`;
- `exclude` с правилом без `reason`, с пустым `when`, с id на `exclude.`
  или с неизвестным полем — ошибка загрузки, каждая со своим сообщением;
- отсев по `attribute` работает: тип с `[GenerateCode]` отсеивается решением,
  которое ни одним шаблоном пути не выразить.

**Проверка**
```bash
uv run pytest tests/test_classify.py tests/test_stats.py -q
uv run docpipe scan --root tests/fixtures/SampleSolution --stats
```

---

## T24 — `docpipe symbols`: что именно осталось без решения

**Цель:** отладка набора правил на одном проекте. Отчёт T23 отвечает «сколько»,
эта задача — «что именно и по чему для него писать предикат».

**Изменить:** `docpipe/stats.py`, `docpipe/emit.py`, `docpipe/cli.py`;
создать `docpipe/explain.py`, `tests/test_explain.py`

### Зачем

Задача пришла из работы на боевом репозитории, из этого шага цикла настройки:
...
Дальше двигаться нечем. Известно, что осталось шесть символов и что четыре из них
не опознаются по имени, — но **не известно, что это за символы**. Настройка идёт
проектами, в каждом остаток в единицы типов, и на этом масштабе нужен не счётчик,
а список: FQN, замыкание наследования, атрибуты, публичные члены, путь. То есть
ровно то, по чему пишется предикат.

До T24 это делалось разовым скриптом поверх внутренних функций `docpipe` — то же,
что уже было с `tools/unclassified.py` до T20. Признак один и тот же: если для
работы по инструкции нужен скрипт, которого нет в инструменте, значит его не хватает.

### Спецификация

`docpipe symbols --root PATH [--config F] [--rules F]` — печатает символы, ничего
не пишет. Фильтры:

| Флаг | Значение |
|---|---|
| `--state` | `undecided` (по умолчанию), `not_documented`, `documented`, `not_enrolled`, `interface_covered`, `any` |
| `--module` | подстрока пути `.csproj`; со `*` — глоб, как в `enrolled` |
| `--namespace` | начало namespace |
| `--rule` | правило, причастное к решению: и отсев, и классификация |
| `--kind` | вид сущности |
| `--limit` | сколько показать; `total` в заголовке остаётся полным |
...
Показываются **не все** поля символа, а решающие. Полный дамп заставляет листать
вывод вместо того, чтобы принять решение; `--format json` есть для тех случаев,
когда нужно остальное.

**Публичные члены в выводе обязательны.** Имена методов grid-сервиса — это контракт
(их вызывают по имени через прокси, см. бизнес-план), и по ним опознаётся вид
сущности там, где имя типа не говорит ничего.

**`--rule` работает и для отсева, и для классификации.** Спрашивают его, чтобы
проверить только что написанное правило, и помнить при этом, в какой оно секции,
человек не должен. На классифицированных он показывает и правила, которые совпали,
но не победили: `--rule service` находит `RiskComputeService`, ставший
`ignite_service`.

### Ловушки

**Выборка и отчёт обязаны считать одно и то же.** Две реализации решения по символу
разошлись бы на первой новой категории, и тогда `--stats` говорит «шесть без
решения», а `symbols` показывает пять — доверять нельзя ни одному. Поэтому решение
вынесено в единственную функцию `stats.decide`, которой пользуются обе. Проверяется
тестом `test_selection_agrees_with_the_report`.

**Индекс символов приходится вернуть из прогона.** `ScanResult` получил поле `index`:
спрашивать «что осталось без решения» у манифеста бессмысленно — туда по определению
попадает только решённое.

**Обрезка срезов — дело вывода, а не подсчёта.** `_top` больше не режет до 15,
`format_breakdown(stats, top)` режет при выводе и помечает остаток («и ещё 476»).
Иначе `--top` пришлось бы протаскивать параметром через `run()` до `collect_stats`.
Заодно исправлено: `most_common(15)` при равных счётчиках сохранял порядок вставки,
то есть порядок обхода индекса; теперь сортировка явная — по убыванию, при равенстве
по имени.

**Опечатка в `--state` обязана быть ошибкой.** Молча пустая выборка выглядит как
«всё решено» — самый неудачный из возможных ответов.

**Команда не пишет ничего.** Её зовут в цикле; кэш разбора при этом используется,
поэтому повторный вызов на том же репозитории дёшев.

**Критерии приёмки**
- `symbols` на фикстуре без флагов печатает `Sample.Pricing.Api.Program` —
  единственный символ без решения;
- `--state` для `undecided`, `not_documented` и `interface_covered` даёт ровно те же
  числа, что соответствующие счётчики `collect_stats`;
- `--module Sample.Common` оставляет только символы этого проекта; то же со глобом
  `src/Sample.Common/**`;
- `--rule service --state documented` находит и `PricingService`, и
  `RiskComputeService` (у второго правило совпало, но не победило);
- `--rule data.contracts --state not_documented` находит `PriceDto`;
- вывод по `ignite_service` содержит путь со строкой, `IService` и имя метода
  `Execute`; у `PricingService` (partial из двух файлов) — пометку `(+1`;
- `--limit 3` при десяти символах: `total` остаётся 10, строк три;
- `--state undecied` — код возврата не 0 и сообщение «неизвестное состояние»;
- `--top 0` в `scan --stats`: срез присутствует, строк не показано, есть «и ещё 1 строка».

**Проверка**
```bash
uv run pytest tests/test_explain.py tests/test_stats.py -q
uv run docpipe symbols --root tests/fixtures/WildSolution
```
...
**Цель:** одна точка входа. Инструкции ссылаются на `docpipe`, а не на скрипт,
который эти же команды пересказывает.

**Изменить:** `deploy/README.md`, `deploy/OFFLINE.md`, `deploy/install.sh`,
`deploy/cashflow-docspipe/{README.md,docpipe.yaml,rules.yaml}`, `CASHFLOW.md`,
`README.md`, `tests/test_deploy_bundle.py`; удалить
`deploy/cashflow-docspipe/run.sh`

### Зачем

`run.sh` завёл свой набор режимов — `stats`, `scan`, `dry-run`, `validate`, `diff`,
`symbols`, `--` — поверх тех же команд CLI. Расплата обнаружилась на практике:
человек, который зовёт `docpipe` напрямую, читает инструкцию про `./run.sh stats`
и не находит в ней ни одного своего флага, а чтобы узнать, какие пути и переменные
подставляются, обязан прочитать bash. Требование было сформулировано так: «лучше
это будет просто списком переменных, а не скриптом, который надо разбирать».

Второй счёт — за поддержку: каждая новая команда требовала строки в `case`, иначе
режима не было (`symbols` добавлялся в T24 в двух местах). Обёртка, которая
перечисляет команды, разъезжается с CLI по умолчанию.

### Что скрипт держал по существу

Три вещи, и все три обязаны сохраниться, потому что каждая иначе даёт отказ
на закрытом контуре:

| Что | Почему |
|---|---|
| `uv run --no-sync` | без него каждый вызов сверяется с индексом и падает там, где индекса нет |
| `python -m docpipe` + `PYTHONPATH` | при установке с `--no-install-project` консольной команды нет вовсе |
| `cd $CF_ROOT` перед вызовом | `out` в конфигурации задан от **текущего каталога**, а не от `--root` |

Всё это переехало в инструкцию списком `export` плюс одна строка `alias`, после
чего команды в документации пишутся ровно так, как их набирает человек:
`docpipe scan --root . --config $BUNDLE/docpipe.yaml --stats`.

### Ловушка

**Требование `--no-sync` осталось без исполнителя.** Пока был скрипт, флаг был
в коде и проверялся тестом по этому коду. Теперь единственный носитель требования —
текст инструкции, поэтому тест проверяет **её**: `--no-sync` обязан присутствовать
...
на машине без доступа к индексу.

Третий носитель — `install.sh`: его финальная подсказка печатает тот же список
переменных и алиас, то есть человек получает рабочий вызов сразу после установки,
не открывая README.

**Критерии приёмки**
- `deploy/cashflow-docspipe/run.sh` отсутствует, `install.sh` его не копирует;
- ни один `.md`, `.sh` или `.yaml` в `deploy/` и `CASHFLOW.md` не упоминает `run.sh`;
- `--no-sync` присутствует в трёх документах поставки — проверяется тестом;
- `python -m docpipe` и `PYTHONPATH` присутствуют в инструкции — проверяется тестом;
- список файлов установщика в `tests/test_deploy_bundle.py` не содержит `run.sh`.

**Проверка**
```bash
uv run pytest tests/test_deploy_bundle.py -q
grep -rn "run.sh" deploy/ CASHFLOW.md README.md    # пусто
```
...
**Цель:** в установленном дереве лежит ровно один набор правил — настроенный.

**Изменить:** `deploy/install.sh`, `deploy/README.md`,
`deploy/cashflow-docspipe/{docpipe.yaml,rules.yaml}`, `CASHFLOW.md`,
`tests/test_deploy_bundle.py`

### Зачем

Установщик клал в `$DEST/rules/dotnet.yaml` копию эталонного набора — «для
сравнения с настроенным». Замечание с боевой машины: рядом уже лежит
`cashflow-docspipe/rules.yaml`, зачем второй.

Проверка показала, что копия не только не нужна, но и вредна.

**Она ничего не давала.** Сравнение «что настроено под АС CF» обеспечивают
пометки «АС CF» в самом наборе, а сравнение «что изменилось выше по течению» —
механизм `keep_configured`: при обновлении новая версия ложится рядом как
`rules.yaml.new`. К тому же после T23 эталон переехал в форму с причинами,
а набор АС CF остался в краткой, и `diff` между ними распух до 68 строк даже
без комментариев — то есть показывал форму записи, а не настройку.

**И она создавала молчаливую ошибку.** Значение `rules` по умолчанию —
`rules/dotnet.yaml` относительно текущего каталога. Прогон из каталога
инструмента без `--config` брал эталонный набор и **успешно завершался**:
манифест построен не теми правилами, диагностики никакой. Проверено
воспроизведением.

### Спецификация

`install.sh` больше не копирует эталон и не создаёт `$DEST/rules`.

Удалить остаток от прежних установок он не может — установщик по правилу не
удаляет ничего, — но обязан о нём **сказать**: если `$DEST/rules/dotnet.yaml`
существует, в stderr печатается предупреждение с объяснением ловушки и командой
`rm -r`. Промолчать здесь нельзя: обновление оставило бы ловушку на месте.

**Критерии приёмки**
- прогон `install.sh <репозиторий> --skip-sync` на чистом каталоге даёт ровно два
  `.yaml` в дереве — `cashflow-docspipe/docpipe.yaml` и
  `cashflow-docspipe/rules.yaml`; каталога `rules/` нет;
- тот же прогон поверх дерева с оставшимся `rules/dotnet.yaml` печатает в stderr
  предупреждение с `rm -r` и файл **не** удаляет;
- ни один файл поставки не ссылается на `../rules/dotnet.yaml`.
