...
**Предикаты над `DocNode`** — одиннадцать, намеренно мало (та же причина, что в
`classify.py`: большой набор означает, что настройщик каждый раз выбирает из десяти
способов написать одно и то же). Восемь — про расположение узла:
...
Три — про контракт типа, те же имена и та же семантика, что в правилах классификации:

| Предикат | По какому полю | Зачем |
|---|---|---|
| `attribute` | `symbol.attributes[].name` | У команды есть маркерный атрибут |
| `base_type` | `symbol.base_types` + `base_types_raw` | База написана в самом объявлении |
| `inherits` | `symbol.base_type_closure` | База может быть через промежуточный тип; обычно нужен именно он |

**Почему они есть, хотя владение — про расположение.** Набор правил классификации
и набор правил владения настраивает один человек, и граница команды сплошь и рядом
описана тем же признаком, что и вид сущности: «наследник `MlApiControllerBase` — наш
контроллер». Без этих трёх предикатов ту же границу пришлось бы выразить вторым
способом, через путь. Два описания одной границы расходятся на первом же файле,
переехавшем в другой каталог, и расхождение ничем не проявляется: документ просто
уезжает не в ту команду.

Трактовка имён — **общая функция** `classify.matches_any_type`, а не своя копия. Иначе
одна и та же строка означала бы в двух файлах разное: `base_type_candidates` разбирает
fluent-базы и квалифицированные имена нетривиально, и вторая реализация разошлась бы
с первой незаметно.

**Ловушка, которой нет у предикатов расположения.** `base_type_closure` строится по
индексу символов прогона, и через модуль, исключённый в `docpipe.yaml`, рвётся его
**транзитивная** часть. Воспроизведено на `SampleSolution` с исключённым `Sample.Common`:
у `PricingController` замыкание остаётся `["BaseApiController"]` — прямая база уцелела,
нерезолвнутая, — а `ControllerBase` из её объявления потерян. Правило по прямой базе
поэтому работает, правило по базе через уровень молча перестаёт. И проявляется это
обычно раньше владения: та же база участвует в правиле классификации, тип уходит
в `undecided` и узлом не становится вовсе — владеть нечем, в `docs owners` его нет.

Порядок проверки — `docpipe symbols --state undecided`, затем `docs owners --lint`.
Это обязано быть в справочнике `docs/ownership.md`: иначе настройщик потратит часы
и решит, что предикат не работает.

**Чего в наборе нет намеренно:** `name_suffix`, `name_regex`, `type_kind`, `modifier`,
`has_member_with_attribute`. Они описывают форму типа, а не границу ответственности,
а владение по имени уже выражается `fqn_prefix`.
...
| M13 | `docpipe worklist`: файл-очередь для внешнего исполнителя шага 3 | M09 |
...
> микрооптимизация: в `.venv` встречаются `.md` на мегабайты. **Но отсев обязан быть
> ровно настолько же терпимым, насколько терпим разбор.** Реализация сравнивала
> `read_bytes()[:4]` с `b"---\n"` — и документ с UTF-8 BOM до `read_document`,
> который специально читает `utf-8-sig`, не доезжал вовсе. Ловушка была обезврежена
> на уровень ниже и заново создана уровнем выше; тест на BOM звал `read_document`
> напрямую и этого не видел. Правило: у отсева и у разбора один и тот же ответ
> на вопрос «это наш файл», иначе разница между ними всегда молчаливая.

> **Ловушка. `missing` означает «файла нет», а не «обход его не вернул».** Разница
> дорогая: `missing` → `create` → `write_atomic` поверх существующего файла, без
> копии и без сообщения — «создано: 1» в отчёте о документе, который переписан.
> Выпасть из обхода можно пятью способами (BOM и прочие формы `---` не первой
> строкой, front matter без `docpipe.schema`, `docs_scan_exclude`, каталог
> за симлинком — `rglob` в такие не заходит, права на файл), и перечислить их
> заранее нельзя: шестой появится вместе со следующей платформой. Поэтому проверка
> не перечисляет причины, а спрашивает диск: `shadowed_docs` сверяет `doc_path`
> узлов без файла с реальным `is_file()`. Совпало — `refuse`/`broken`, а не `create`,
> и `content` не собирается вовсе, чтобы и `--force` не перезаписал: что лежит
> в этом файле, инструменту неизвестно. Плюс структурный запрет в самой записи —
> `create` поверх существующего файла — на случай плана, собранного мимо проверки.

> **Ловушка. `docs_scan_exclude` может накрыть само дерево документов.** Пара
> `docs_root` + `modules_dir` держит инвариант «пишем туда, где ищем» структурно,
> и этого казалось достаточно. Не достаточно: `docs_scan_exclude: ["docs/ml/**"]`
> из конфигурации поставки отсекает все документы, как только `docs_root` окажется
> под `docs/ml` (а `business_root` там уже лежит). Симптом — прогон, в котором
> **все** документы `missing` и переписываются каждый раз, молча. Проверка
> сравнивает шаблоны с реальными `doc_path` манифеста, а не с префиксом: так она
> не зависит от того, как записан шаблон. Находка блокирующая — прогон, который
> гарантированно затирает всё дерево, продолжать нельзя.
...
1. `--action` сужает и `counts`, и `total` — они считаются по **отфильтрованному**
   набору. Сводка «что с деревом целиком» и очередь «что отдать агенту» в одном
   её выводе не помещаются: либо честная сводка вместе с тысячами `current`
   в списке, либо очередь без сводки;
2. у её JSON нет конверта — ни версии формата, ни привязки к манифесту. Чужой
   процесс не отличит формат следующей версии от текущего и не проверит,
   что читает файл от того самого `doc-tree.json`.

Дописать конверт в `docs status` нельзя, не сломав её потребителей, а версионировать
человекочитаемую команду — значит запретить менять её вывод. Поэтому `docs status`
остаётся информационной командой для человека и CI, `worklist` — версионированным
артефактом для машины. Общая у них ровно одна вещь — запись документа
(`status._document_json`), и она обязана остаться общей: две реализации разойдутся
на первом же новом поле, и агент начнёт видеть разные причины у одного документа.
...
```json
{
  ...
  "totals": {"documents": 1856, "selected": 20, "truncated": false},
  "documents": [
    {
      "action": "write",
      "reason": "контракт изменился: добавлены RecalculateAsync",
      "status": "stale",
...
Записи документов — байт в байт те же, что у `docs status --format json`: та же
функция, то же множество полей.

`counts` — по **всему** плану, включая `current` и всё, что отфильтровано.
`totals.documents` — сколько документов в дереве, `totals.selected` — сколько попало
...
равенстве — по `doc_path`. Это отличается от `docs status`, где сортировка только
по `doc_path`, и отличается намеренно: без приоритета `--limit` резал бы очередь
по алфавиту, отдавая агенту `Api/…` вместо сломанного документа.

### Модель и схема

`Worklist` и `WorklistEntry` — `pydantic`, `frozen=True`, `extra="forbid"`, как всё
остальное. Схема — производная от моделей, `docpipe schema --model worklist --out
schema/doc-worklist.schema.json`; у `--model` два значения, `doc-tree` (по умолчанию,
прежнее поведение) и `worklist`. Отдельно поддерживаемый файл схемы разошёлся бы
с кодом — ровно то, ради чего команда `schema` и существует.

`schema_version` — версия **формата очереди**, она своя и не связана с
`Manifest.schema_version`.

### Поведение

Файл пишется всегда, когда план построен, — в том числе когда очередь пуста.
Пустой список означает «работы нет», отсутствие файла — «прогон не состоялся»,
и путать эти два состояния внешнему модулю нельзя.

При блокирующих ошибках плана (`plan.errors`) файл **не пишется и не переписывается**,
ошибки идут в stderr, код 1. Прежний файл остаётся нетронутым.

Коды возврата: **0** — файл записан, **1** — блокирующие ошибки плана,
**2** — ошибка пользователя.

> **Ловушка. Времени в файле нет.** Соблазн положить `generated_at`, чтобы внешний
> модуль видел свежесть, стоит дороже, чем даёт: файл начинает меняться при каждом
> прогоне, его нельзя ни сравнить между прогонами, ни закоммитить без шума, и тест
> на детерминизм превращается в сравнение с исключением полей. Свежесть определяется
> по `manifest_sha256`, а время файла — по файловой системе. То же правило, по которому
> недетерминированное шага 1 живёт в `doc-tree.run.json`, а не в манифесте.

> **Ловушка. `worklist` не пишет в дерево документации ничего.** Он не вызывает
> `materialize` и не чинит front matter. Иначе получаем вторую реализацию записи —
> и она разойдётся с `apply` на первом же документе с чужими ключами front matter.

> **Ловушка. `missing` в очереди — это документ, которого на диске ещё нет.** Агент,
> получивший `doc_path` и пошедший его читать, получит «файла нет» и решит, что
> очередь битая. Поэтому в конверте есть `needs_materialize` — «в очереди есть
> документы, для которых сначала нужен `materialize`». Считается как наличие статуса
> `missing` в **очереди**, а не во всём плане: отфильтрованный `missing` агента
...
> `(_STATUS_ORDER.index(status), doc_path)`; опираться на порядок из плана нельзя —
> он свой и меняется вместе с `docs status`.

> **Ловушка. Запись документа не копировать.** `status._document_json` вынести
> в публичную `document_json` и звать из обоих мест. Копия разойдётся с оригиналом
> при первом же новом поле, и одна и та же ситуация получит в двух отчётах разные
> причины — а причина здесь единственное, по чему внешний модуль решает, что делать.

**Критерии приёмки**
- на золотом дереве сразу после `materialize`: файл записан, `totals.selected` = 6,
  все `action` = `write`, `needs_materialize` = false (документы созданы, статус
  `empty`, а не `missing`), код 0;
- до `materialize` на пустом дереве: 6 записей со статусом `missing`,
  `needs_materialize` = true;
- два прогона подряд дают файл, совпадающий **байт в байт**;
- `counts` не меняется при `--action write`, `--team` и `--limit`; меняется только
  `totals.selected` и `documents`;
- `--limit 2` при трёх разных статусах отдаёт две записи в порядке приоритета
  статуса, `totals.truncated` = true;
- запись документа в `worklist` и соответствующая запись в
  `docs status --format json` совпадают по всем полям;
- после `docs accept` всех документов очередь пуста, файл **записан**,
...
| `test_worklist.py` | Конверт и версия схемы; `counts` по всему дереву при суженной очереди; порядок и `--limit`; байт-в-байт на повторе; отказ не переписывает файл |
...
23. **В файле очереди (M13) нет времени.** Свежесть — `manifest_sha256`; `generated_at`
    сделал бы файл меняющимся на каждом прогоне.
24. **Сводка в очереди считается по всему дереву**, фильтры сужают только список.
    Иначе `--team` объявит, что в дереве двадцать документов.
...
- любая интеграция с LLM и написание текста документации — это шаг 3. M13 отдаёт
  шагу 3 **очередь**, а не исполнителя: сам обход очереди, вызов агента и запись
  текста живут в другом проекте;
- бизнес-документы в очереди M13. У бизнес-слоя свой набор статусов (нет `missing`
  и `orphan`) и нет `sources`, поэтому общий формат либо потеряет половину полей
  у обеих сторон, либо станет объединением с необязательными полями — и внешний
  модуль всё равно будет ветвиться по слою. `business status --format json` остаётся
  отдельным выходом; когда очередь понадобится и там, она делается по образцу M13
  своей командой;
