...
## Манифест фронта — тот же формат

Шаг `web` пишет **отдельный** файл (`doc-tree.web.json`) той же схемы:
...
| `web_calls[]` | HTTP-вызовы: файл, строка, ключ `(метод, маршрут, различитель)`, уверенность, **член**, `via_action` |
| `uses[]` | граф вызовов: `target`, `member` цели, `via` (член источника), `action` — тип экшена, если дошли диспатчем |
| `absorbed_by` | страница, внутри документа которой узел описывается; пусто — свой документ |
