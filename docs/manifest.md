...
|---|---|---|
| идентичность | `id`, `title`, `signature_hash` | |
...
`type_parameters`, `attributes`, `xml_doc`, `ambiguous`, плюс три важных:
...
```
docs/modules/{имя модуля}/{вид}s/{slug}.md
```
...
- **Шаг 2** читает манифест и создаёт (или сверяет) иерархию каталогов по
  `doc_path`, плюс рендерит индексы `docs/_views/` — карту по доменам, таблицу
  эндпоинтов, список сервисов кластера. Представления — чистая функция от
  манифеста, поэтому не протухают;
