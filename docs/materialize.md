...
по умолчанию — `templates`. **Путь отсчитывается от текущего каталога, а при
отсутствии — от каталога `docpipe.yaml`**, как `rules`, `ownership`
и `registries`, но не как `out`: у целей записи второй ступени нет. От `--root`
он не отсчитывается никогда. В репозитории разработки разницы не видно —
команды и так зовутся из корня, — а в установке, где инструмент лежит не в корне
сканируемого репозитория, значение по умолчанию указывает в никуда, и
`materialize` отказывается работать: «Каталог шаблонов не найден». Ошибка про
путь, причина в конфигурации. Полная карта баз отсчёта — в
...
Документы бэкенда лежат под `docs_root`/`modules_dir`, документы фронта —
под `docs_root`/`web.modules_dir` (пустое значение — та же ветка). Обход
документов идёт от `docs_root` и находит обе; сверка префикса выбирает
ожидаемый корень по манифесту, поэтому манифест фронта не роняет прогон
на репозитории, где ветки разведены. Подробнее — в [`pages.md`](pages.md).
