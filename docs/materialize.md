...
по умолчанию — `templates`. **Путь ищется сначала от текущего каталога, затем
от каталога самого `docpipe.yaml`** — так же разрешаются `rules`, `registries`
и `ownership`. От `--root` не отсчитывается ни один из них.

Порядок ступеней важен и обратным быть не может: конфигурации, лежащие в корне
репозитория, писались с путями от текущего каталога, и отдай приоритет каталогу
конфигурации — прогон не упал бы, а взял другой набор шаблонов, ничего не сказав.

Вторая ступень нужна установке, где инструмент лежит не в корне сканируемого
репозитория: путь внутри конфигурации естественно писать относительно неё же,
и до появления ступени такая конфигурация давала «Каталог шаблонов не найден» —
ошибку про путь при исправной раскладке. Не найдено нигде — сообщение называет
значение из конфигурации, а не последний перебранный путь.

`out` этой ступени не имеет намеренно: это цель записи, а не поиска. «Первый
существующий» для неё ничего не значит, а угадывать, куда писать, инструмент
не должен.
